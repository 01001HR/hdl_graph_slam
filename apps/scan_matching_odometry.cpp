...
public:
  ...
  void onInit() {
    ROS_INFO_STREAM("initializing scan_matching_odometry...");

    initialize_params();

    points_sub = nh.subscribe("/filtered_points", 256, &ScanMatchingOdometry::cloud_callback, this);
...
private:
  ...
  void initialize_params() {
    ...
    base_frame_id = private_nh.param<std::string>("base_frame_id", "velodyne");
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    if(transform_thresholding) {
      ...
      if(dx > max_acceptable_trans || da > max_acceptable_angle) {
        ROS_INFO_STREAM("too large transform!!  " << dx << "[m] " << da << "[rad]");
        ROS_INFO_STREAM("ignore this frame(" << stamp << ")");
        return keyframe_pose * prev_trans;
      }
    }
